...
2. Making such model reusable, configurable and extensible, so that it could be used in different settings and with different data sources.
...
An analysis of the exchange of data between the patients as a whole and the hospital system was made basically impossible and, for this reason, we decided to focus on modelling the interactions of each single patient with the hospital system and to avoid.
