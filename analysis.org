...
- For all distributions concerning activities that have undergone a grouping procedure, an HyperStar output file was created for each group (see [] for specification of the hyperstar output) and one mapping file to track the association between the group and the classes.
...
- A HyperStar output file for each class for all distributions regarding activities that have undergone partial classification.
# - Un singolo file output di Hyperstar per le attività che non hanno subito una classificazione
- A single Hyperstar output file for activities that have not undergone classification.
...
- For each exponential distribution used to fit the events in MIMIC, a file bearing the estimated lambda parameters for each class.
...
- For exponential distributions used to fit waveforms, a file bearing the estimated lambda parameters for each class.
...
- For each of the two classifications made (patient based and admission based, seen in section []) a file containing the probability for a patient or admission to belong to one or the other class.
...
- For each decision node in the patient activity diagram presented in Figure [], a file containing the probabilities (for each patient class and admission class) of takinf one decision branch rather than the other.
...
- A single file containing, for each possible signal recorded by the waveforms, the probability for each class that such signal is transmitted.
