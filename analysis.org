...
Table [] shows the results of the grouping method described above considering an alpha limit of 0.05 applied on each kind of behavior on which a grouping was meant to be applied, showing in particular the reduction in the number of classes to be considered for the fitting through HyperStar.
